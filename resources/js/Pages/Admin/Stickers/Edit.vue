...
let breadcrumbs = ref([
  ...
  { title: "Sticker", disabled: true, href: "stickers.index" },
]);
</script>

<template>
  <Head title="Edit sticker" />
  ...
  <AdminLayout>
    ...
    <v-sheet class="pa-4" rounded="xl" elevation="3">
      <div class="w-50 mx-auto">
        <h2 class="text-center mt-2">Edit sticker</h2>
        <form @submit.prevent="form.post(route('stickers.update', sticker))">
          <v-text-field
            v-model="form.title"
            label="Title"
            variant="underlined"
